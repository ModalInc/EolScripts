...
{   // Script Class
    ...
    public partial class RoamingAttacker : Bot
    {
        ...
        public Vector3 steerForPersuePlayer(InfantryVehicle vehicle)
        {
            ...
            List<Vehicle> RoamingAttackers = _arena.getVehiclesInRange(vehicle.state.positionX, vehicle.state.positionY, 500,
                                                                delegate (Vehicle v)
                                                                { return (v is RoamingAttacker); });
            IEnumerable<IVehicle> RoamingAttackersBots = RoamingAttackers.ConvertAll<IVehicle>(
                delegate (Vehicle v)
                {
                    return (v as RoamingAttacker).Abstract;
                }
            );

            Vector3 seperationSteer = vehicle.SteerForSeparation(_seperation, -0.707f, RoamingAttackersBots);
