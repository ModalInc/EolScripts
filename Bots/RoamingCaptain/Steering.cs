...
{   // Script Class
    ...
    public partial class RoamingCaptain : Bot
    {
        ...
        public Vector3 steerForPersuePlayer(InfantryVehicle vehicle)
        {
            ...
            List<Vehicle> roamingCaptains = _arena.getVehiclesInRange(vehicle.state.positionX, vehicle.state.positionY, 500,
                                                                delegate (Vehicle v)
                                                                { return (v is RoamingCaptain); });
            IEnumerable<IVehicle> roamingCaptainBots = roamingCaptains.ConvertAll<IVehicle>(
                delegate (Vehicle v)
                {
                    return (v as RoamingCaptain).Abstract;
                }
            );

            Vector3 seperationSteer = vehicle.SteerForSeparation(_seperation, -0.707f, roamingCaptainBots);
