...
{   // Script Class
    ...
    public partial class BasicDefense : Bot
    {
        ...
        public Vector3 steerForPersuePlayer(InfantryVehicle vehicle)
        {
            ...
            List<Vehicle> Defenders = _arena.getVehiclesInRange(vehicle.state.positionX, vehicle.state.positionY, 500,
                                                                delegate (Vehicle v)
                                                                { return (v is BasicDefense); });
            IEnumerable<IVehicle> DefenderBots = Defenders.ConvertAll<IVehicle>(
                delegate (Vehicle v)
                {
                    return (v as BasicDefense).Abstract;
                }
            );

            Vector3 seperationSteer = vehicle.SteerForSeparation(_seperation, -0.707f, DefenderBots);
        ...
        public Vector3 steerForFollowOwner(InfantryVehicle vehicle)
        {
            ...
            Vector3 seek = vehicle.SteerForSeek(_targetPoint.position());
