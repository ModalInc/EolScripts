...
{   // Script Class
    ...
    public partial class AlienChief : Bot
    {
        ...
        public Vector3 steerForPersuePlayer(InfantryVehicle vehicle)
        {
            ...
            List<Vehicle> roamingChiefs = _arena.getVehiclesInRange(vehicle.state.positionX, vehicle.state.positionY, 500,
                                                                delegate (Vehicle v)
                                                                { return (v is AlienChief); });
            IEnumerable<IVehicle> roamingChiefBots = roamingChiefs.ConvertAll<IVehicle>(
                delegate (Vehicle v)
                {
                    return (v as AlienChief).Abstract;
                }
            );

            Vector3 seperationSteer = vehicle.SteerForSeparation(_seperation, -0.707f, roamingChiefBots);
        ...
        /// </summary>
        public Vector3 steerForPoint(InfantryVehicle vehicle)
        {
            ...
            Vector3 seekSteer = vehicle.SteerForSeek(pos = new Vector3(((float)_targetPoint.positionX) / 100.0, ((float)_targetPoint.positionY) / 100.0, 0));
